...
class RecurrentRGCN(nn.Module):
    ...
    def forward(self, g_list, static_graph, use_cuda):
        ...
        for i, g in enumerate(g_list):
            ...
            for span, r_idx in zip(g.r_len, g.uniq_r):
                x = temp_e[span[0]:span[1],:] # embeddings of entities concated by relation r
            ...
            time_weight = torch.sigmoid(torch.mm(self.h, self.time_gate_weight) + self.time_gate_bias) # U_t
            self.h = time_weight * current_h + (1-time_weight) * self.h
            self.h = F.normalize(self.h)
            history_embs.append(self.h)
            logits, labels = self.h.mm(temp.t()), torch.arange(self.h.size(0)).to(self.h.device)

            evolve_loss.append(self.criterion(logits, labels))
        return history_embs, static_emb, self.h_0, gate_list, degree_list, sum(evolve_loss) / len(evolve_loss)
    ...
    def get_loss(self, glist, triples, static_graph, use_cuda):
        """
        :param glist: (num_history_graphs, dglgraph)
        :param triplets: (num_triples, 3)
        :param static_graph: 
        :param use_cuda:
        :return:
        """
        ...
        evolve_embs, static_emb, r_emb, _, _, evolve_loss = self.forward(glist, static_graph, use_cuda)
        ...
        if self.use_static:
            if self.discount == 1:
                ...
                for time_step, evolve_emb in enumerate(evolve_embs):
                    ...
                    mask = (math.cos(step) - sim_matrix) > 0
                    loss_static += self.weight * torch.sum(torch.masked_select(math.cos(step) - sim_matrix, mask))
        return loss_ent, loss_rel, loss_static, evolve_loss
