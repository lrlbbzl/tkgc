...
class GNN(nn.Module):
    def __init__(self, in_dim, out_dim, layer_num, num_rel, num_head, gnn='rgcn', att_drop=0.2, fea_drop=0.2,):
...
class RGAT(nn.Module):
    def __init__(self, in_dim, out_dim, att_drop, fea_drop):
        ...
        self.self_loop = nn.Parameter(torch.Tensor(out_dim, out_dim))
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_uniform_(self.self_loop, gain=gain)
    ...
    def reduce_func(self, nodes):
        ...
        h = self.feature_drop(torch.sum(alpha * (nodes.mailbox['h'] + nodes.mailbox['r_h']), dim=1) + torch.mm(nodes.data['h'], self.self_loop))
        return {'h': h}
